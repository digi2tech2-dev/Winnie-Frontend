...
const timelineSteps = [
  ["PENDING", "Order created", "The backend has recorded this order."],
  ["PROCESSING", "Processing", "The order is being handled by the platform or provider."],
  ...
  useEffect(() => {
    ...
    const loadOrder = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setError(requestError.userMessage || "Unable to load order details.");
          setOrder(null);
        }
      } finally {
  ...
  if (error || !order) {
    return (
      <EmptyState
        ...
        description={error || "This backend order could not be loaded."}
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
      <section className="glass-panel rounded-lg p-6">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          <Info label="Status" value={order.statusLabel} />
        ...
        {Object.keys(customerValues).length > 0 && (
          <div className="mt-8">
            <h2 className="text-xl font-black">Submitted details</h2>
      ...
      <aside className="glass-panel rounded-lg p-6">
        ...
        <div className="mt-5 space-y-4">
          {timelineSteps.map(([status, title, text]) => {
            ...
            return (
              <div key={status} className="flex gap-3">
                ...
                <span>
                  ...
                  <span className="text-sm text-slate-400 dark:text-[#8A94A7]">{text}</span>
                </span>
              </div>
            );
          })}
