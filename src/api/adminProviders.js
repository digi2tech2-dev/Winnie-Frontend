...
export function normalizeFazerCardsImportPreview(preview = {}) {
  preview = preview || {};
  return {
    autoPriceSyncAvailable: preview.autoPriceSyncAvailable === true,
    blockReason: preview.blockReason || "",
    calculatedLocalPrice: preview.calculatedLocalPrice === undefined || preview.calculatedLocalPrice === null ? "" : String(preview.calculatedLocalPrice),
    calculatedPriceSource: preview.calculatedPriceSource || "",
    costPrice: preview.costPrice ?? "",
    currency: String(preview.currency || DEFAULT_CURRENCY).toUpperCase(),
    defaultMarkupType: preview.defaultMarkupType || "percentage",
    defaultMarkupValue: toNumber(preview.defaultMarkupValue, 0),
    defaultPricingMode: preview.defaultPricingMode || "",
...
function normalizeImportedProduct(product = {}) {
  ...
  return {
    ...
    name: safeValue.name || "",
    finalPrice: safeValue.finalPrice === undefined || safeValue.finalPrice === null ? "" : String(safeValue.finalPrice),
    markupType: safeValue.markupType || "",
    markupValue: toNumber(safeValue.markupValue, 0),
    pricingMode: safeValue.pricingMode || "",
    providerPrice: safeValue.providerPrice === undefined || safeValue.providerPrice === null ? "" : String(safeValue.providerPrice),
