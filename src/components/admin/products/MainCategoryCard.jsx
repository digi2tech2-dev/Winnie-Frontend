  ...
  return (
    <article className="group overflow-hidden rounded-[22px] border border-slate-200/90 bg-white shadow-[0_12px_30px_rgba(15,23,42,0.055)] transition hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-[0_18px_38px_rgba(124,58,237,0.10)] dark:border-white/[0.08] dark:bg-[#111827] dark:shadow-[0_0_18px_rgba(139,92,246,0.09)] dark:hover:border-violet-400/30">
      <div className="relative h-28 overflow-hidden sm:h-32">
        ...
        <span className={`absolute left-2.5 top-2.5 inline-flex items-center gap-1 rounded-full border border-white/20 px-2 py-1 text-[9px] font-black text-white backdrop-blur-md ${category.visible ? "bg-emerald-500/75" : "bg-slate-700/75"}`}>
          {category.visible ? <Eye className="h-3 w-3" /> : <EyeOff className="h-3 w-3" />}
          {category.visible ? "ظاهر" : "مخفي"}
        </span>
        <h3 className="absolute bottom-3 right-3 left-3 truncate text-base font-black text-white">{category.name}</h3>
      </div>
      <div className="flex items-center gap-2 p-3">
        <span className="inline-flex min-w-0 flex-1 items-center gap-1.5 text-[10px] font-black text-slate-500 dark:text-[#9AA7BD]">
          <Hash className="h-3.5 w-3.5 text-violet-500" />
          ترتيب العرض: {category.displayOrder.toLocaleString("ar-EG")}
        </span>
