...
export default function SubCategoryCard({ category, parentName, onEdit, onDelete }) {
  return (
    <article className="flex items-center gap-3 rounded-[20px] border border-slate-200/90 bg-white p-2.5 shadow-[0_10px_26px_rgba(15,23,42,0.05)] transition hover:-translate-y-0.5 hover:border-sky-200 dark:border-white/[0.08] dark:bg-[#111827] dark:shadow-[0_0_16px_rgba(139,92,246,0.08)]">
      <img src={category.image} alt={category.name} className="h-16 w-16 shrink-0 rounded-2xl object-cover" />
      <div className="min-w-0 flex-1">
        <h4 className="truncate text-xs font-black text-slate-950 dark:text-white">{category.name}</h4>
        <p className="mt-1 flex items-center gap-1 truncate text-[9px] font-bold text-slate-500 dark:text-[#9AA7BD]">
