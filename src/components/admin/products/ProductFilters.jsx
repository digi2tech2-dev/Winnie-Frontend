  ...
  return (
    <form onSubmit={onSearch} className="px-1 sm:px-2">
      <div className="flex flex-wrap items-center gap-2">
        <label className="relative min-w-[220px] flex-1 sm:max-w-sm"><span className="pointer-events-none absolute left-1 top-1 grid h-8 w-8 place-items-center rounded-md border border-violet-500/40 bg-violet-500/15 text-violet-300"><Search className="h-4 w-4" /></span><input type="text" value={filters.query} onChange={update("query")} aria-label="البحث في المنتجات" placeholder="ابحث باسم المنتج" className={`${inputClassName} py-0 pl-11 pr-3`} /></label>
        <button type="button" onClick={() => setOpen((value) => !value)} className={`inline-flex h-10 items-center gap-2 rounded-lg border px-3 text-[11px] font-black transition ${open || activeCount ? "border-fuchsia-500/70 bg-fuchsia-500/15 text-fuchsia-200" : "border-[#1a2e5b] bg-[#060d23] text-slate-300 hover:border-violet-500/60"}`}><Filter className="h-4 w-4" />فلترة{activeCount > 0 && <span className="grid h-5 min-w-5 place-items-center rounded-full bg-violet-500 px-1 text-[9px] text-white">{activeCount}</span>}</button>
      </div>
      {open && <div className="mt-3 grid gap-2 rounded-xl border border-[#172b58] bg-[#02091d] p-3 sm:grid-cols-2 xl:grid-cols-5">
...
function Select({ children, ...props }) { return <select {...props} className={`${inputClassName} px-3`}>{children}</select>; }
